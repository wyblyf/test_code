...
func Write_to_file(sid abi.SectorID, out []byte, path string) {
	p := Out1{
		Sid: sid,
		Out: out,
	}
...
func Read_from_file(path string) (Out1, error) {
	...
	app := &cli.App{
		...
		Flags: []cli.Flag{
			...
			&cli.StringFlag{
				Name:  "phase2",
				Value: "",
				Usage: "only phase2 process from storage-dir",
			},
			///////////////////// FEAT2000 PATCH END  /////////////////////
			&cli.StringFlag{
				Name:  "storage-dir",
		...
		Action: func(c *cli.Context) error {
			...
			numSectors := abi.SectorNumber(c.Int("task"))
			chs := make([]chan int, c.Int("task"))
			for i := abi.SectorNumber(1); i <= numSectors && robench == ""; i++ {
				go func(ch chan int, x int) error {
					path := sbdir + "/" + strconv.Itoa(int(x))
					...
					///////////////////// ARS FEAT2000 PATCH /////////////////////
					if c.String("phase2") != "" {
						path = c.String("storage-dir") + "/" + strconv.Itoa(int(x))
						log.Info("Read_from_file: ", path, "/phase1.json")
						data, err := Read_from_file(path + "/phase1.json")
						if err != nil {
							return err
						}

						log.Info("Running replication(2)...")
						log.Info("sid.Miner:", data.Sid.Miner, " sid.Number:", data.Sid.Number, " pc1o:", data.Out)
						sbfs = &basicfs.Provider{
							Root: path,
						}
						...
						sb.SealPreCommit2(context.TODO(), data.Sid, data.Out)
						return err
					}
					///////////////////// FEAT2000 PATCH END  /////////////////////
					...
					if err != nil {
						...
						log.Info("sid.Miner:", sid.Miner, " sid.Number:", sid.Number, " pc1o:", pc1o)
						Write_to_file(sid, pc1o, path+"/phase1.json")
						return nil
					}
